...
def writeRec7():
    ...
    SIGMA=np.array(levsIncl)/1013.25 #Sigma-p values for each vertical layer (pressure/reference pressure)
    for s in SIGMA:
        fout.write('{:6.3f}\n'.format(s))
...
#####PARAMETERS
startDate='20180306' #Eventually this will be passed in as an argument
...
inDir=os.getenv("HOME")+'/UNRESP_ndrive/Data/NAM_20180306' #Parent Directory containing GRIB files
nfiles=17 #Number of GRIB files (files are 3 hourly, so 48 hours is 17 files including hours 0 and 48)
outFile=os.getenv("HOME")+'/Data/UNRESP/3D.DAT' #Outpuf ile path
